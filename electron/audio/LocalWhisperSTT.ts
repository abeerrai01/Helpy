...
export class LocalWhisperSTT extends EventEmitter {
     ...
     * IN FLIGHT (flushed here, queued behind a loading worker, or already
     * in-flight from a prior streaming tick) — the Answer/Stop tail wait uses
     * this to keep the recording gate open for the seconds a local model needs,
     * instead of the short grace it gives a channel with nothing pending.
     *
     * The `streamingTaskInFlight` check covers the race where the user presses
     * Stop while the streaming loop has ALREADY dispatched the open segment to
     * the worker (so vad.flush() finds nothing new), but the worker hasn't
     * replied yet. Without it, that case returned false and the renderer only
     * waited TAIL_GRACE_MS (1s) — far too short for local Whisper (5-7s).
     */
    finalize(): boolean {
