...
export class LLMHelper {
  ...
  public isOpenRouterKey(key?: string | null): boolean {
    if (!key) return false;
    const k = key.trim();
    return k.startsWith('sk-or-') || k.toLowerCase().includes('openrouter');
  }
  ...
  constructor(apiKey?: string, useOllama: boolean = false, ollamaModel?: string, ollamaUrl?: string, groqApiKey?: string, openaiApiKey?: string, claudeApiKey?: string, deepseekApiKey?: string, nvidiaNimApiKey?: string, geminiFallbackApiKeys?: string[] | string) {
    ...
    // Initialize OpenAI / OpenRouter client if API key provided
    if (openaiApiKey) {
      this.openaiApiKey = openaiApiKey
      const isOpenRouter = this.isOpenRouterKey(openaiApiKey)
      this.openaiClient = new OpenAI({
        ...
        ...(isOpenRouter ? {
          ...
          defaultHeaders: {
            ...
            'X-Title': 'Natively AI Assistant'
          }
        } : {})
      })
      console.log(`[LLMHelper] ${isOpenRouter ? 'OpenRouter' : 'OpenAI'} client initialized with model: ${isOpenRouter ? OPENROUTER_DEFAULT_MODEL : OPENAI_MODEL}`)
    }
  ...
  public setOpenaiApiKey(apiKey: string) {
    ...
    const isOpenRouter = this.isOpenRouterKey(trimmed);
    this.openaiClient = new OpenAI({
      apiKey: trimmed,
      ...(isOpenRouter ? {
        baseURL: OPENROUTER_BASE_URL,
    ...
    console.log(`[LLMHelper] ${isOpenRouter ? 'OpenRouter' : 'OpenAI'} API Key updated.`);
  }
  ...
  private isOpenAiModel(modelId: string): boolean {
    ...
    if (this.isOpenRouterKey(this.openaiApiKey)) {
      if (modelId.startsWith('openrouter/') || (modelId.includes('/') && !this.isLiteLLMModel(modelId) && !this.isNvidiaNimModel(modelId))) {
        return true;
      }
    }
  ...
  public resolveOpenAiModel(modelId?: string): string {
    const isOpenRouter = this.isOpenRouterKey(this.openaiApiKey);
    if (modelId) {
    ...
    if (this.isOpenAiModel(this.currentModelId)) {
      if (isOpenRouter && !this.currentModelId.includes('/') && this.currentModelId.startsWith('gpt-')) {
        return `openai/${this.currentModelId}`;
      }
      return this.currentModelId;
    }
    return isOpenRouter ? OPENROUTER_DEFAULT_MODEL : OPENAI_MODEL;
  }
  ...
  private buildTextSpareRungs(
    ...
    if (!skip.has('openai') && this.openaiClient) {
      const textOpenAI = this.resolveOpenAiModel();
      const isOpenRouter = this.isOpenRouterKey(this.openaiApiKey);
      const name = isOpenRouter ? `OpenRouter (${textOpenAI})` : `OpenAI (${textOpenAI})`;
      spares.push({
        id: 'openai', name, isLocal: false, priority: prio++,
        open: (sig) => this.streamWithOpenai(userContent, finalSystemPrompt, textOpenAI, sig),
      });
    }
    return spares;
  }
...
let isMultimodal = !!(imagePaths?.length);
  ...
  private async generateWithOpenai(userMessage: string, systemPrompt?: string, imagePaths?: string[], modelId?: string): Promise<string> {
    ...
    // Use explicit override, then current model if it's OpenAI/OpenRouter, else baseline constant
    const model = this.resolveOpenAiModel(modelId);
    ...
    const rawMax = model.toLowerCase().includes('claude') ? this.getClaudeMaxOutput(model) : getOpenAiMaxOutput(model, MAX_OUTPUT_TOKENS);
    const max_completion_tokens = this.isOpenRouterKey(this.openaiApiKey) ? Math.min(rawMax, OPENROUTER_MAX_OUTPUT_TOKENS) : rawMax;
    const request = {
      model,
      messages,
    ...
    try {
      ...
      if (this.isOpenRouterKey(this.openaiApiKey) && (err?.status === 402 || (err?.message && (err.message.includes('credits') || err.message.includes('afford'))))) {
        console.warn(`[LLMHelper] OpenRouter credit limit reached on ${model}. Retrying with ${OPENROUTER_DEFAULT_MODEL} / ${OPENROUTER_FREE_MODEL}...`);
        try {
          response = await this.withTimeout(
  ...
  public async * streamChatWithGemini(message: string, imagePaths?: string[], context?: string, skipSystemPrompt: boolean = false, abortSignal?: AbortSignal, outcome?: { incomplete?: boolean; reason?: 'output_cap' | 'provider_died_post_commit' }): AsyncGenerator<string, void, unknown> {
    ...
    if (isMultimodal) {
      ...
      if (this.openaiClient) {
        const effectiveOpenAI = this.resolveOpenAiModel(textOpenAI);
        const openaiLabel = this.isOpenRouterKey(this.openaiApiKey) ? 'OpenRouter' : 'OpenAI';
        providers.push({ name: `${openaiLabel} (${effectiveOpenAI})`, execute: () => this.streamWithOpenaiMultimodal(userContent, imagePaths!, openaiSystemPrompt, effectiveOpenAI, abortSignal) });
      }
      if (this.client) {
      ...
      if (this.openaiClient) {
        const effectiveOpenAI = this.resolveOpenAiModel(textOpenAI);
        const openaiLabel = this.isOpenRouterKey(this.openaiApiKey) ? 'OpenRouter' : 'OpenAI';
        providers.push({ name: `${openaiLabel} (${effectiveOpenAI})`, execute: () => this.streamWithOpenai(userContent, openaiSystemPrompt, effectiveOpenAI, abortSignal) });
      }
      if (this.claudeClient) {
    ...
    const currentFamilyLabel = this.currentModelId === 'natively' ? 'Natively'
      : this.isClaudeModel(this.currentModelId) ? 'Claude'
        : this.isOpenAiModel(this.currentModelId) ? (this.isOpenRouterKey(this.openaiApiKey) ? 'OpenRouter' : 'OpenAI')
  ...
  private async *streamVisionWithFallback(
    ...
    if (!localOnly) {
      if (this.openaiClient) {
        const isOpenRouter = this.isOpenRouterKey(this.openaiApiKey);
        const name = isOpenRouter ? 'OpenRouter' : 'OpenAI';
        cloud.push({ id: 'openai', name, isLocal: false, priority: prio++, ttftTimeoutMs: FLASH_TTFT_MS,
          open: (sig, att) => this.streamWithOpenaiMultimodal(userContent, imagePaths, systemPrompt, this.resolveOpenAiModel(tierModel(ModelFamily.OPENAI, att)), sig) });
      }
      if (this.claudeClient) {
  ...
  private async * _streamChatInner(
    ...
    // 7. Last-resort: OpenAI / OpenRouter client. Reached when Gemini, Natively
    // and all configured custom providers have failed or are absent. A user who
    // stored their OpenRouter key in the OPENAI_API_KEY field (common when
    // following the OpenRouter "drop-in replacement" docs) will hit this path
    // every time Gemini is selected as the primary but the Gemini key is expired
    // or missing. Without this rung the chain throws "No AI provider configured"
    // even though a perfectly good OpenRouter key is stored.
    //
    // Guards:
    //  • isLocalOnlyMode: cloud-only provider — skip in local-only mode.
    //  • isMultimodal && imagePaths: text-only path; images travel the vision
    //    chain (streamVisionWithFallback). A text-only fallback answering an
    //    image question would silently drop the screenshot, so skip it here.
    //  • openaiClient getter already returns null if the 'openai' provider
    //    family is disabled by the user in Settings → AI Providers.
    if (this.openaiClient && !this.isLocalOnlyMode && !(isMultimodal && imagePaths)) {
      const textOpenAI = this.resolveOpenAiModel();
      const isOpenRouter = this.isOpenRouterKey(this.openaiApiKey);
      const providerLabel = isOpenRouter ? `OpenRouter (${textOpenAI})` : `OpenAI (${textOpenAI})`;
      console.log(`[LLMHelper] All primary providers failed — falling back to ${providerLabel}.`);
      try {
        yield* this.trackCommit(this.streamWithOpenai(userContent, finalSystemPrompt, textOpenAI, abortSignal), commit);
        return;
      } catch (e: any) {
        if (commit.emitted) {
          console.warn(`[LLMHelper] ${providerLabel} last-resort failed AFTER first token — ending stream: ${e?.message || e}`);
          yield LLMHelper.TRUNCATION_SENTINEL;
          return;
        }
        console.warn(`[LLMHelper] ${providerLabel} last-resort failed: ${e?.message || e}`);
      }
    }
  ...
  private async * streamWithOpenai(userMessage: string, systemPrompt?: string, modelId?: string, abortSignal?: AbortSignal): AsyncGenerator<string, void, unknown> {
    ...
    // Use explicit override, then currentModelId if it's an OpenAI/OpenRouter model, else baseline constant
    const model = this.resolveOpenAiModel(modelId);
    ...
    let stream: any;
    try {
      stream = await this.openaiClient.chat.completions.create(request, { signal: abortSignal });
    } catch (err: any) {
      if (this.isOpenRouterKey(this.openaiApiKey) && (err?.status === 402 || (err?.message && (err.message.includes('credits') || err.message.includes('afford'))))) {
        console.warn(`[LLMHelper] OpenRouter credit limit reached on ${model}. Retrying with ${OPENROUTER_DEFAULT_MODEL} / ${OPENROUTER_FREE_MODEL}...`);
        try {
  ...
  private async * streamWithOpenaiMultimodal(userMessage: string, imagePaths: string[], systemPrompt?: string, modelId?: string, abortSignal?: AbortSignal): AsyncGenerator<string, void, unknown> {
    ...
    try {
      ...
      if (this.isOpenRouterKey(this.openaiApiKey) && (err?.status === 402 || (err?.message && (err.message.includes('credits') || err.message.includes('afford'))))) {
        console.warn(`[LLMHelper] OpenRouter credit limit reached on vision model ${model}. Retrying with ${OPENROUTER_DEFAULT_MODEL}...`);
        stream = await this.openaiClient.chat.completions.create({
          ...request,
          model: OPENROUTER_DEFAULT_MODEL,
          max_completion_tokens: 1024,
