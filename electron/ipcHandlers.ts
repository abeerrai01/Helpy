...
export function initializeIpcHandlers(appState: AppState): void {
  ...
  safeHandle('start-mic-recording', async () => {
    await appState.startMicRecording();
  });
  ...
  safeHandle('get-stored-credentials', async () => {
    try {
      ...
      return {
        ...
        geminiPreferredModel: creds.geminiPreferredModel || cm.getPreferredModel('gemini') || undefined,
  ...
  safeHandle(
    ...
    async (
      ...
      try {
        ...
        let validatedImagePaths: string[] = [];
        ...
        // Vision-first: run ScreenUnderstandingService
        // If imagePaths was not provided, captureIfMissing: true automatically takes a screenshot
        try {
          const {
            getScreenUnderstandingService,
          } = require('./services/screen/ScreenUnderstandingService');
          ...
          const sur = await sus.understand({
            ...
            qualityMode: 'balanced',
            imagePaths: validatedImagePaths.length > 0 ? validatedImagePaths : undefined,
          ...
          if (sur.imagePaths && sur.imagePaths.length > 0 && validatedImagePaths.length === 0) {
            validatedImagePaths = sur.imagePaths;
          }
          screenContextStatus =
        ...
        const answer = await intelligenceManager.runWhatShouldISay(
          question,
          0.8,
          validatedImagePaths.length > 0 ? validatedImagePaths : undefined,
          {
