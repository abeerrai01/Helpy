...
// Ensure Cargo binary directory (~/.cargo/bin or D:\cargo\bin) is in PATH if cargo is installed there
const candidateCargoDirs = [
  path.join(os.homedir(), '.cargo', 'bin'),
  'D:\\cargo\\bin',
];
if (fs.existsSync('D:\\rustup') && !process.env.RUSTUP_HOME) {
  process.env.RUSTUP_HOME = 'D:\\rustup';
}
if (fs.existsSync('D:\\cargo') && !process.env.CARGO_HOME) {
  process.env.CARGO_HOME = 'D:\\cargo';
}
for (const cargoBinDir of candidateCargoDirs) {
