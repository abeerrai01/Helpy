...
const NativelyInterface: React.FC<NativelyInterfaceProps> = ({
  ...
  // resolved yet.
  const [currentModelDisplayName, setCurrentModelDisplayName] = useState<string>('Gemini 3.8 Flash');
  ...
  // Auto-dismiss the "Microphone is silent" banner when audio recovers.
  // main.ts sends 'audio-capture-recovered' whenever the zero-fill detector
  // in wireMicCapture observes peakToPeak > 100 after having already triggered
  // the stuck banner — meaning the mic came back to life (user unmuted,
  // privacy pane re-enabled, etc.). Without this the banner stays on screen
  // indefinitely until the user manually closes it.
  useEffect(() => {
    const unsub = window.electronAPI?.onAudioCaptureRecovered?.((payload) => {
      setSystemAudioWarning((current) => {
        // Only clear warnings that match the recovered channel so a concurrent
        // system-audio warning isn't silently dismissed by a mic recovery event.
        if (
          current?.kind === 'audio-capture-failure' &&
...
Provide only the answer, nothing else.`;
      ...
      // Ensure the mic capture pipeline is initialized and running.
      // setupSystemAudioPipeline() is idempotent — during an active meeting it
      // is already done; outside a meeting this is the trigger that creates
      // MicrophoneCapture + googleSTT_User so the Answer button produces a
      // transcript instead of "No speech detected".
      try {
        await window.electronAPI.startMicRecording();
      } catch (err) {
        console.warn('[NativelyInterface] startMicRecording failed (mic may already be running):', err);
      }
    }
  };
