...
// Windows Microphone Consent Probe — runs once per renderer window startup.
//
// On Windows, WASAPI (our Rust native audio capture) delivers zero-filled silence
// if this Electron process has not yet acquired a Windows microphone consent. The
// OS grants consent when the renderer calls getUserMedia({audio:true}) — this is
// the standard Chromium path for registering a live mic use with the Windows
// Privacy system (Settings → Privacy → Microphone). We only request the stream
// to acquire the consent; we stop all tracks immediately. A second request from
// any window in the same process is a no-op from the OS's perspective — the
// grant is process-wide once acquired. Guarded to the launcher window only to
// avoid N parallel probes for N renderer windows (overlay, settings, etc.).
(async () => {
  try {
    const platform = window.electronAPI?.platform ?? (typeof process !== 'undefined' ? process.platform : '');
    const windowParam = new URLSearchParams(window.location.search).get('window') ?? '';
    const isLauncher = !windowParam || windowParam === 'launcher';
    if (platform === 'win32' && isLauncher) {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
      stream.getTracks().forEach(t => t.stop()); // immediately release; we only needed the consent
      console.log('[renderer] Windows mic consent acquired via getUserMedia probe');
    }
  } catch (micErr: any) {
    // NotAllowedError = user explicitly denied or Windows Privacy is off.
    // NotFoundError = no mic device. Both are logged but must not crash the app.
    console.warn('[renderer] Windows mic consent probe failed (banner will explain):', micErr?.name ?? micErr?.message ?? String(micErr));
  }
})();
