 ...
 * the block is the brace-balanced region starting from the outer
 * `if (!zerofillLatched)` guard that wraps the peak-to-peak detection.
 *
 * Note: the original guard was `if (!zerofillLatched && !zerofillTriggered)`.
 * After the recovery refactor the outer guard became `if (!zerofillLatched)`
 * so that recovery can run even after the trigger fires; `!zerofillTriggered`
 * moved inside as a guard on the trigger-only branch. The extraction still
 * targets the outer if-block so all the peak-to-peak assertions apply.
 */
function extractZerofillBlock(body) {
  // Try the new (post-recovery) guard first, fall back to the legacy form
  // so the test suite stays green whether this file is run on old or new code.
  let marker = '!zerofillLatched && !zerofillTriggered';
  let idx = body.indexOf(marker);
  if (idx < 0) {
    // Post-recovery shape: outer guard is just `!zerofillLatched`.
    marker = '!zerofillLatched)';
    idx = body.indexOf(marker);
  }
  assert.ok(idx >= 0, 'expected the zerofill guard expression (!zerofillLatched[...] block)');
