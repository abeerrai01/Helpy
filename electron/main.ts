...
import { ensureWindowsFullDrag } from "./utils/windowsDragPolicy"
...
export class AppState {
  ...
  constructor() {
    ...
    this.isUndetectable = settingsManager.get('isUndetectable') ?? (process.platform === 'win32' ? true : false);
    ...
    if (process.platform === 'win32' && this.isUndetectable) {
      ensureWindowsFullDrag();
    }
  ...
  private createSTTProvider(speaker: 'interviewer' | 'user'): STTProvider | null {
    const { CredentialsManager } = require('./services/CredentialsManager');
    let sttProvider = CredentialsManager.getInstance().getSttProvider();
    const sttLanguage = CredentialsManager.getInstance().getSttLanguage();

    const createLocalWhisper = (): STTProvider => {
    ...
    const hasGoogleCredentials = (): boolean => {
      ...
      const p = creds.getGoogleServiceAccountPath?.();
      return Boolean(p && !creds.isPlaceholderKey?.(p));
    };

    const safeFallback = (reason: string): STTProvider => {
      if (hasGoogleCredentials()) {
        console.warn(`[Main] ${reason}, falling back to GoogleSTT`);
        return new GoogleSTT(speaker);
      }
      console.warn(`[Main] ${reason}, falling back to LocalWhisperSTT`);
      return createLocalWhisper();
    };

    // Auto-resolve 'none' to LocalWhisperSTT so audio capture and real-time transcription work out-of-the-box
    if (sttProvider === 'none') {
    ...
    if (sttProvider === 'natively') {
      ...
      if (!nativelyKey) {
        stt = safeFallback(`No Natively API Key configured for ${speaker}`);
      } else {
      ...
      if (apiKey) {
        ...
        stt = safeFallback(`No API key for Deepgram STT`);
      }
      ...
      if (apiKey) {
        ...
        stt = safeFallback(`No API key for Soniox STT`);
      }
      ...
      if (apiKey) {
        ...
        stt = safeFallback(`No API key for ElevenLabs STT`);
      }
      ...
      if (apiKey) {
        ...
        stt = safeFallback(`No API key for OpenAI STT`);
      }
      ...
      if (key) {
        ...
        stt = safeFallback(`No NVIDIA NIM API key for ${speaker}`);
      }
      ...
      if (apiKey) {
        ...
        stt = safeFallback(`No API key for ${sttProvider} STT`);
      }
    } else if (sttProvider === 'local-whisper') {
      stt = createLocalWhisper();
    } else {
      stt = safeFallback(`Fallback provider`);
    }
    ...
    stt.on('transcript', (segment: { text: string, isFinal: boolean, confidence: number, speakerId?: string }) => {
      // User (mic) transcripts: always forward regardless of meeting state.
      // The Answer button works in standalone/launcher mode — the user presses
      // Answer, speaks, and presses Stop outside any meeting. The renderer's
      // own isRecordingRef guard prevents user chunks from leaking into chat
      ...
      if (!zerofillLatched) {
        ...
        if (peakToPeak > 100) {
          if (zerofillTriggered) {
            // Recovery: audio resumed after the "Microphone is silent" banner was
            // shown. Reset state and dismiss the banner on the renderer side.
            console.log(`${prefix}Mic audio recovered (peakToPeak=${peakToPeak}) — clearing silent-mic banner.`);
            zerofillTriggered = false;
            firstChunkAt = 0;
            this.sendToMeetingSurfaces('audio-capture-recovered', { channel: 'mic' });
          }
  ...
  private async setupSystemAudioPipeline(): Promise<void> {
    ...
    try {
      ...
      if (!this.googleSTT) {
        ...
        if (!this.googleSTT) {
          try {
            console.log('[Main] Falling back to LocalWhisperSTT for interviewer channel');
            const { LocalWhisperSTT } = require('./audio/LocalWhisperSTT');
            const sm = SettingsManager.getInstance();
            const modelId = sm.get('localWhisperModel') ?? 'Xenova/whisper-tiny.en';
            const lws = new LocalWhisperSTT(modelId);
            lws.setChannel('system');
            this.googleSTT = lws as any;
          } catch (fbErr) {
            console.error('[Main] LocalWhisperSTT fallback failed for interviewer:', fbErr);
          }
        }
      ...
      if (!this.googleSTT_User) {
        ...
        if (!this.googleSTT_User) {
          try {
            console.log('[Main] Falling back to LocalWhisperSTT for microphone channel');
            const { LocalWhisperSTT } = require('./audio/LocalWhisperSTT');
            const sm = SettingsManager.getInstance();
            const modelId = sm.get('localWhisperModel') ?? 'Xenova/whisper-tiny.en';
            const lws = new LocalWhisperSTT(modelId);
            lws.setChannel('mic');
            this.googleSTT_User = lws as any;
          } catch (fbErr) {
            console.error('[Main] LocalWhisperSTT fallback failed for mic:', fbErr);
          }
        }
  ...
  public setUndetectable(state: boolean): void {
    ...
    if (process.platform === 'win32') {
      if (state) ensureWindowsFullDrag();
...
async function initializeApp() {
  ...
  // Windows Microphone Privacy Gate — MUST run immediately after whenReady().
  //
  // On Windows, WASAPI (which our Rust native capture module uses) will deliver
  // zero-filled silent audio if the app does not hold an active Windows
  // microphone consent. Consent is acquired by the OS when a renderer in this
  // Electron process calls navigator.getUserMedia({audio:true}) and the user
  // (or Windows auto-grants for previously-allowed apps) accepts. Without this
  // the Chromium media permission handler blocks the call and the Rust thread
  // reads silence forever — which is exactly the "Microphone Is Silent" / zero-
  // fill scenario observed on Windows.
  //
  // setPermissionRequestHandler allows the renderer call to reach Windows.
  // setPermissionCheckHandler (the pre-check that Electron 20+ runs before
  // even firing the request) must also return true, otherwise Chromium
  // short-circuits with a "permission denied" before Windows sees it.
  if (process.platform === 'win32') {
    try {
      ...
      ses.setPermissionCheckHandler((_wc, permission) => {
        ...
        return true; // default-allow everything; the app's renderer is trusted
      });
      ...
      ses.setPermissionRequestHandler((_wc, permission, callback) => {
        ...
        callback(true); // allow all other renderer-requested permissions
      });

      console.log('[Init] Windows microphone permission handler installed on default session');
    } catch (permErr: any) {
      console.warn('[Init] Failed to install Windows mic permission handler (non-fatal):', permErr?.message || permErr);
    }
  }
