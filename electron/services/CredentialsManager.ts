...
export class CredentialsManager {
     ...
     */
    public isPlaceholderKey(key: string | undefined): boolean {
        if (!key) return true;
    ...
    private storedOrEnv(stored: string | undefined, envKey: string): string | undefined {
        ...
        if (value && !this.isPlaceholderKey(value)) return value;
        ...
        if (fromEnv && !this.isPlaceholderKey(fromEnv)) return fromEnv;
        return undefined;
    }
    ...
    public getSttProvider(): 'none' | 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'nvidia_nim' | 'natively' | 'local-whisper' {
        let provider = this.credentials.sttProvider || 'none';
        // Auto-heal / auto-resolve: if provider is 'none', resolve to the best available STT provider
        if (provider === 'none') {
            if (this.getNativelyApiKey()) {
                provider = 'natively';
            } else if (this.getDeepgramApiKey()) {
                provider = 'deepgram';
            } else if (this.getGroqSttApiKey()) {
                provider = 'groq';
            } else if (this.getOpenAiSttApiKey()) {
                provider = 'openai';
            } else if (this.getSonioxApiKey()) {
                provider = 'soniox';
            } else if (this.getElevenLabsApiKey()) {
                provider = 'elevenlabs';
            } else if (this.getAzureApiKey()) {
                provider = 'azure';
            } else if (this.getIbmWatsonApiKey()) {
                provider = 'ibmwatson';
            } else if (this.getGoogleServiceAccountPath() && !this.isPlaceholderKey(this.getGoogleServiceAccountPath())) {
                provider = 'google';
            } else {
                // Free, on-device transcription without external API keys
                provider = 'local-whisper';
            }
            this.credentials.sttProvider = provider;
            if (this.keyringUnreadable) {
                console.log(`[CredentialsManager] Auto-resolved sttProvider in memory only (${provider})`);
            } else {
                this.saveCredentials();
                console.log(`[CredentialsManager] Auto-resolved sttProvider: none→${provider}`);
            }
            return provider;
        }
        return provider;
    ...
    public getDeepgramApiKey(): string | undefined {
        return this.storedOrEnv(this.credentials.deepgramApiKey, 'DEEPGRAM_API_KEY');
    }

    public getGroqSttApiKey(): string | undefined {
        return this.storedOrEnv(this.credentials.groqSttApiKey || this.credentials.groqApiKey, 'GROQ_STT_API_KEY')
            || this.getGroqApiKey();
    }
    ...
    public getOpenAiSttApiKey(): string | undefined {
        return this.storedOrEnv(this.credentials.openAiSttApiKey || this.credentials.openaiApiKey, 'OPENAI_STT_API_KEY')
    ...
    public getElevenLabsApiKey(): string | undefined {
        return this.storedOrEnv(this.credentials.elevenLabsApiKey, 'ELEVENLABS_API_KEY');
    }

    public getAzureApiKey(): string | undefined {
        return this.storedOrEnv(this.credentials.azureApiKey, 'AZURE_SPEECH_KEY');
    }
    ...
    public getIbmWatsonApiKey(): string | undefined {
        return this.storedOrEnv(this.credentials.ibmWatsonApiKey, 'IBM_WATSON_API_KEY');
    }
    ...
    public getSonioxApiKey(): string | undefined {
        return this.storedOrEnv(this.credentials.sonioxApiKey, 'SONIOX_API_KEY');
    }
    ...
    public getOpenrouterApiKey(): string | undefined {
        const direct = this.storedOrEnv(this.credentials.openrouterApiKey, 'OPENROUTER_API_KEY');
        if (direct) return direct;
        const openaiKey = this.getOpenaiApiKey();
        if (openaiKey && (openaiKey.startsWith('sk-or-') || openaiKey.toLowerCase().includes('openrouter'))) {
            return openaiKey;
        }
        return undefined;
    }
    ...
    public getNativelyApiKey(): string | undefined {
        return this.storedOrEnv(this.credentials.nativelyApiKey, 'NATIVELY_API_KEY');
    }
    ...
    public getPreferredModel(provider: PreferredModelProvider): string | undefined {
        ...
        if (provider === 'gemini') {
            return this.storedOrEnv(this.credentials[key] as string | undefined, 'GEMINI_PREFERRED_MODEL')
                || this.storedOrEnv(this.credentials[key] as string | undefined, 'DEFAULT_MODEL')
                || (this.credentials[key] as string | undefined);
        }
