...
type Provider = 'gemini' | 'groq' | 'openai' | 'claude' | 'deepseek' | 'nvidia_nim' | 'openrouter';
...
async function fetchOpenAIModels(apiKey: string): Promise<ProviderModel[]> {
    if (apiKey.startsWith('sk-or-') || apiKey.toLowerCase().includes('openrouter')) {
        return fetchOpenRouterModels(apiKey);
    }
