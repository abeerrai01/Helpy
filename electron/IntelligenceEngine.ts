...
export class IntelligenceEngine extends EventEmitter {
    ...
    private async runWhatShouldISayInner(question?: string, confidence: number = 0.8, imagePaths?: string[], options?: { speculative?: boolean; skipCooldown?: boolean; screenContext?: ScreenContext; promptInstruction?: string; activeSkill?: { id: string; name: string; promptBlock: string }; domContext?: string; forceFresh?: boolean }): Promise<string | null> {
        ...
        try {
            ...
            const visualContextQuestion = (() => {
                const sc = options?.screenContext as any;
                if (sc) {
                    if (sc.taskDetected && typeof sc.taskDetected === 'string' && sc.taskDetected.trim()) {
                        return sc.taskDetected.trim();
                    }
                    if (sc.visibleSummary && typeof sc.visibleSummary === 'string' && sc.visibleSummary.trim()) {
                        return `Analyze and answer based on this screen: ${sc.visibleSummary.trim()}`;
                    }
                    if (sc.extractedText && typeof sc.extractedText === 'string' && sc.extractedText.trim()) {
                        const snippet = sc.extractedText.trim().slice(0, 300);
                        return `Solve or answer based on the screen content: ${snippet}`;
                    }
                }
                if (options?.domContext && options.domContext.trim()) {
                    return 'Analyze the coding problem or content on the screen and provide the solution.';
                }
                if ((imagePaths?.length ?? 0) > 0) {
                    return 'Analyze what is displayed on the screen and provide the answer or solution.';
                }
                return undefined;
            })();

            const wtaResolvedQuestionForKicks = question || extractedQuestion.latestQuestion || lastInterviewerTurn || visualContextQuestion;
            const intentPromise = classifyIntent(
                question || extractedQuestion.latestQuestion || lastInterviewerTurn || visualContextQuestion,
                preparedTranscript,
                this.session.getAssistantResponseHistory().length
            ...
            try {
                ...
                const _wtaQHoist = question || extractedQuestion.latestQuestion || lastInterviewerTurn || visualContextQuestion || '';
            ...
            const wtaTurnQuestion = question || extractedQuestion.latestQuestion || lastInterviewerTurn || visualContextQuestion || '';
            try {
